...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    b.legalNeighbors.foldLeft(Stream[(Block, List[Move])]()) { (s, blockMove) => (blockMove._1, blockMove._2 :: history) #:: s }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream()
      ...
      val (block, history) #:: tail = initial
      val more = newNeighborsOnly(neighborsWithHistory(block, history), explored)
      initial.head #:: (from(tail ++ more, explored ++ more.map { case (block, _) => block }))
    }
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart filter { case (block, hist) => done(block) }
